...
def main():
    ...
    tarefa1 = event_loop.create_task(gerar_dados(total, dados))
    tarefa2 = event_loop.create_task(gerar_dados(total, dados))
    tarefa3 = event_loop.create_task(processar_dados(total * 2, dados))

    tarefas = asyncio.gather(tarefa1, tarefa2, tarefa3)

    event_loop.run_until_complete(tarefas)
